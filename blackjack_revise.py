...
def check_blackjack(cards_Player):
    """
    Check if either the Player or Dealer has a Blackjack (total of 21) 
    and print the result.

    Parameters:
    cards_Player (dict): A dictionary with keys "Player" and "Dealer",
                        each mapped to a list or tuple where the second element [1] 
                        is the total card value.

    Returns:
    bool: True if at least one player has Blackjack, False otherwise.
    """
    winner_list = []
...
def check_highNumber(cards_Player):
    """
    Compare the total card values of the Player and Dealer, and print the result.
    ...
    Returns:
    None: Prints the result based on which total is higher.
    """
...
while game_run:
    ...
    print("Dealer draw card for next round: ")
